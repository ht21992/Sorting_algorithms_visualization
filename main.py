...
def show_popup(msg, window_title="Information"):
    popup = Tk()
...
def draw_data(data, colorlist):
    ...
    for i, height in enumerate(normalized_data):
        ...
        canvas.create_rectangle(
            x0, y0, x1, y1, fill=colorlist[i], outline=gradient_color
        )
...
def generate():
    try:
        ...
        draw_data(data, ["#B22222" for x in range(len(data))])
    except ValueError:
        show_popup(
            f"Min Value must be less than or equal Max Value", window_title="Error"
        )
...
def start_algorithm():
    ...
    if algorithm_menu.get() == "Quick Sort":
        elapsed_time_fl = quick_sort(
            data, 0, len(data) - 1, draw_data, speed_rate.get()
        )
        draw_data(data, ["green" for x in range(len(data))])
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
    elif algorithm_menu.get() == "Bubble Sort":
        elapsed_time_fl = bubble_sort(data, draw_data, speed_rate.get())
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
    elif algorithm_menu.get() == "Merge Sort":
        elapsed_time_fl = merge_sort(data, draw_data, speed_rate.get())
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
    elif algorithm_menu.get() == "Insertion Sort":
        elapsed_time_fl = insertion_sort(data, draw_data, speed_rate.get())
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
    elif algorithm_menu.get() == "Selection Sort":
        elapsed_time_fl = selection_sort(data, draw_data, speed_rate.get())
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
    elif algorithm_menu.get() == "Heap Sort":
        elapsed_time_fl = heap_sort(data, draw_data, speed_rate.get())
        show_popup(
            f"Selected Algorithm: {current_algorithm.get()}\n\nExecution Time: {elapsed_time_fl}"
        )
...
# frame / base layout
UI_frame = Frame(root, width=620, height=200, bg="#2F4F4F")
UI_frame.grid(row=0, column=0, padx=10, pady=5)

canvas = Canvas(root, width=620, height=380, bg="black", highlightbackground="white")
canvas.grid(row=1, column=0, padx=10, pady=5)

# Draw Row[0]
Label(UI_frame, text="Algorithm: ", bg="#8FBC8F").grid(
    row=0,
    column=0,
    padx=5,
    pady=5,
...
algorithm_menu = ttk.Combobox(
    UI_frame,
    state="readonly",
    textvariable=current_algorithm,
    values=[
        "Bubble Sort",
        "Quick Sort",
        "Merge Sort",
...
Label(UI_frame, text="Delay (seconds)", bg="#8FBC8F").grid(
...
Button(
    ...
    fg="white",
    font=("Comic Sans MS", 8, "bold"),
    activebackground="#05945B",
).grid(row=0, column=5, padx=5, pady=5)


# Draw Row[1]
Label(UI_frame, text="Size ", bg="#8FBC8F").grid(
...
Label(UI_frame, text="Min Value ", bg="#8FBC8F").grid(
...
Label(UI_frame, text="Max Value ", bg="#8FBC8F").grid(
